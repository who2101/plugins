...
public void OnPluginStart()
{
	...
	else if(gEV_Type != Engine_SourceSDK2006 && gEV_Type != Engine_TF2) SetFailState("This plugin was meant to be used in CS:S, CS:GO and TF2 *only*.");
...
{
	if(StrEqual(name, "shavit-zones")) gB_Zones = false;
	else if(StrEqual(name, "shavit-replay-playback")) gB_ReplayPlayback = false;
	else if(StrEqual(name, "shavit-rankings")) gB_Rankings = false;
	else if(StrEqual(name, "shavit-hud")) gB_HUD = false;
	else if(StrEqual(name, "eventqueuefix")) gB_Eventqueuefix = false;
	else if (StrEqual(name, "adminmenu"))
	{
		gB_AdminMenu = false;
		gH_AdminMenu = null;
...
{
	// styles
	if(!LoadStyles()) SetFailState("Could not load the styles configuration file. Make sure it exists (addons/sourcemod/configs/shavit-styles.cfg) and follows the proper syntax!");

	// messages
	if(!LoadMessages()) SetFailState("Could not load the chat messages configuration file. Make sure it exists (addons/sourcemod/configs/shavit-messages.cfg) and follows the proper syntax!");
}
...
public Action Command_Style(int client, int args)
{
	...
	else if(menu.ItemCount <= ((gEV_Type == Engine_SourceSDK2006)? 9:8))
	{
...
public int Native_CanPause(Handle handler, int numParams)
{
	...
	if(gEV_Type != Engine_TF2)
	{
		...
		if(gEV_Type == Engine_SourceSDK2006)
		{
