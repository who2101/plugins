...
{
	if(gCV_HideRadar.BoolValue && gEV_Type == Engine_SourceSDK2006)
	{
...
public Action Timer_Scoreboard(Handle timer)
{
	for(int i = 1; i <= MaxClients; i++)
	{
		...
		if(gEV_Type != Engine_SourceSDK2006) UpdateClanTag(i);
	}
...
void UpdateClanTag(int client)
{
	...
	if(gEV_Type != Engine_SourceSDK2006) CS_SetClientClanTag(client, sCustomTag);
...
public void Weapon_Fire(Event event, const char[] name, bool dB)
{
	...
	if (StrContains(sWeapon, "usp") != -1 || StrContains(sWeapon, "hkp") != -1 || StrContains(sWeapon, "glock") != -1)
		SetMaxWeaponAmmo(client, GetEntPropEnt(client, Prop_Data, "m_hActiveWeapon"), true);
}
...
public void Shavit_OnFinish(int client)
{
	...
	UpdateScoreboard(client);
	if(gEV_Type != Engine_SourceSDK2006) UpdateClanTag(client);
}
