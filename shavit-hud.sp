...
public void OnPluginStart()
{
	...
	else if (gEV_Type == Engine_SourceSDK2006) HookUserMessage(gI_HintText, Hook_HintText, true);
...
public Action Hook_GunTouch(int entity, int client)
{
	if (1 <= client <= MaxClients)
	{
		...
		{
			if (!(gI_HUD2Settings[client] & HUD2_USPSILENCER) != (gEV_Type == Engine_SourceSDK2006))
			{
				return Plugin_Continue;
			}

			int state = (gEV_Type == Engine_SourceSDK2006) ? 1 : 0;
...
void GivePlayerDefaultGun(int client)
{
	...
	{
		strcopy(sWeapon, 32, (gEV_Type == Engine_SourceSDK2006) ? "weapon_usp" : "weapon_usp_silencer");
	}
	else
	{
