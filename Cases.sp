...
public void OnPluginStart() 
{
	...
	LoadTranslations("Cases.phrases.txt");
...
#endif
	HookConVarChange((g_hGiveExp = CreateConVar("sm_opener_give_exp","0","Give the experience [1 - Yes | 0 - No]",0, true, 0.0, true, 1.0)), OnConvarChanged);
	bGiveExp = g_hGiveExp.BoolValue;

	HookConVarChange((g_hMinExp = CreateConVar("sm_opener_min_exp","0","Minimum number of received experience.",0)), OnConvarChanged);
	iMinExp = g_hMinExp.IntValue;

	HookConVarChange((g_hMaxExp = CreateConVar("sm_opener_max_exp","0","Maximum number of received experience.",0)), OnConvarChanged);
	iMaxExp = g_hMaxExp.IntValue;
	HookConVarChange((g_hResetCounter = CreateConVar("sm_opener_reset_counter","0","Allow admins to reset the counter [1 - Yes | 0 - No]",0, true, 0.0, true, 1.0)), OnConvarChanged);
	...
	HookConVarChange((g_hTimeBeforeNextOpen = CreateConVar("sm_opener_time_before_next_open", "10800", "Time between case openings in seconds.",0)), OnConvarChanged);
	...
	HookConVarChange((g_hMaxCredits = CreateConVar("sm_opener_max_credits","5000","Maximum number of credits received.",0)), OnConvarChanged);
...
public Action Command_Case(int client, int args) 
{
	...
	{
		if(bCaseMessages) CPrintToChat(client, "%t%t", "prefix", "be_alive");
...
{
	if(!CheckCommandAccess(client, "sm_delete", ADMFLAG_ROOT, true))
	{
		if(bCaseMessages) CPrintToChat(client, "%t%t", "prefix", "not_works");
		EmitSoundToClient(client, "buttons/blip1.wav", SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, -1, NULL_VECTOR);
	}
...
public void OnClientDisconnect(int client) 
{
	if(!IsFakeClient(client)) 
	{
		...
		for(int edict = 0;edict <= 4; edict++) 
			if(iEntCaseData[client][edict] != -1) RemoveEntity(iEntCaseData[client][edict]);		
	}
}
...
{
	for(int i = 0;i < sizeof(sDownloadPaths); i++) AddFileToDownloadsTable(sDownloadPaths[i]);	
		...
		if(iEntCaseData[activator][1] == iEntity && iEntity > MaxClients)
		{
			...
			switch (iReward[activator]) 
			{
				case 0: 
				{
					...
					if(bCaseMessages)
					{
						if(bPrintAll) CPrintToChatAll("%t%t", "prefix", "received_credits_all", activator, iEntCaseData[activator][4]);
						else CPrintToChat(activator, "%t%t", "prefix", "received_credits", iEntCaseData[activator][4]);
					}
				...
				case 1: 
				{
					if(bGiveExp) 
					{
						...
						if(bCaseMessages) 
						{
							if(bPrintAll) CPrintToChatAll("%t%t", "prefix", "received_exp_all", activator, iEntCaseData[activator][4]);
							else CPrintToChat(activator, "%t%t", "prefix", "received_exp", iEntCaseData[activator][4]);
						}
					...
					if(bGiveVIP) 
					{
						if(!VIP_IsClientVIP(activator)) 
						{
							...
							if(bCaseMessages) 
							{
								if(bPrintAll) 
								{
									if(kv.GetNum("time") == 0) CPrintToChatAll("%t%t", "prefix", "got_vip_all_forever", activator, buffer);
									else CPrintToChatAll("%t%t", "prefix", "got_vip_all", activator, buffer, kv.GetNum("time"));
								}
							}
						...
						else 
						{
							if(bCaseMessages) 
							{
								if(bPrintAll) CPrintToChatAll("%t%t", "prefix", "nothing", activator);
								else CPrintToChat(activator, "%t%t", "prefix", "already_has_vip");
							}
			...
			if(IsValidEdict(iEntCaseData[activator][1])) 
			{
				...
				if(iEntCaseData[activator][2] && IsValidEdict(iEntCaseData[activator][2])) RemoveEntity(iEntCaseData[activator][2]);
				RemoveEntity(iEntCaseData[activator][1]);
...
public void Get10Rating(Database db, DBResultSet results, const char[] error, any data)
{
	if(results != INVALID_HANDLE && !error[0])
	{
		if(results.HasResults) 
		{
			...
			else CPrintToChat(data, "Rating is empty now");
		}
	}
	...
	if(result != INVALID_HANDLE && !error[0])
	{
		if(result.HasResults) 
		{
			if(result.RowCount > 0) 
			{
				...
				if(bCaseMessages) CPrintToChat(client, "%t%t", "prefix", "counter_reseted");
			}
		}
	}
			...
			if(result.RowCount > 0)
			{
					...
					iEntCaseData[client][4] == -1 ) 
				{
					if(bCaseAccess) 
					{
						...
						if(AdminID == INVALID_ADMIN_ID) 
						{
							if(bCaseMessages) CPrintToChat(client, "%t%t", "prefix", "not_admin");
							EmitSoundToClient(client, "buttons/blip1.wav", SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, -1, NULL_VECTOR);
							return;
						}
					}
					if(!IsFakeClient(client)) 
					{
						...
						if(TR_DidHit(hTrace) && hTrace != INVALID_HANDLE) 
						{
							...
							{
								if(bCaseMessages) CPrintToChat(client, "%t%t", "prefix", "same_level_case");
							}
							else if(GetVectorDistance(fClientOrigin, fEndOfTrace) > float(iMaxPositionValue * 100) && bMaxPosition) 
							{
								if(bCaseMessages) CPrintToChat(client, "%t%t", "prefix", "too_longer", iMaxPositionValue);
								if(bOutputBeam) 
								{
	...
	if(result != INVALID_HANDLE && !error[0])
	{
		if(result.HasResults) 
		{
			if(result.RowCount > 0) 
			{
				...
				if(time >= 0) CPrintToChat(client, "%t%t", "prefix", "wait_next_case", time/3600/24, time/3600%24, time/60%60, time%60);
				EmitSoundToClient(client, "buttons/blip1.wav", SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, -1, NULL_VECTOR);
...
public Action SoundOpen(Handle hNewTimer, int client) 
{
	...
	switch(iReward[client]) 
	{
		case 0:
		{
			...
			if(bCaseMessagesHint) PrintHintText(client, "%t", "credits_scroll", iEntCaseData[client][4]);
		}
		case 1:
		{
			while(iEntCaseData[client][4] == -1) iEntCaseData[client][4] = GetRandomInt(iMinExp,iMaxExp);
			if(bGiveExp) if(bCaseMessagesHint) PrintHintText(client, "%t", "exp_scroll", iEntCaseData[client][4]);
			else if(bCaseMessagesHint) PrintHintText(client, "%t", "credits_scroll", iEntCaseData[client][4]);
		}
#if defined _vip_core_included
		case 2:
		{
			if(bGiveVIP)
			{
				...
				{
					hArrayList.GetString(iEntCaseData[client][4], buffer,sizeof(buffer))
					PrintHintText(client, "%t", "vip_scroll", buffer);
				}
			}
			else 
			{
...
public Action OnTouchDelete(Handle hNewTimer, int activator) 
{
	...
	RemoveEntity(iEntCaseData[activator][0]);
...
void PrintToHintScrolling(int client) 
{
	...
	switch(Random)
	{
		case 0: PrintHintText(client, "%t", "credits_scroll", GetRandomInt(iMinCredits,iMaxCredits));

		case 1: 
		{
			if(bGiveExp) PrintHintText(client, "%t", "exp_scroll", GetRandomInt(iMinExp,iMaxExp));
			else PrintHintText(client, "%t", "credits_scroll", GetRandomInt(iMinCredits,iMaxCredits));
		}
...
void SpawningReward(float fPos[3], int client) 
{
	...
	if(iEntCaseData[client][0] != -1) AcceptEntityInput(iEntCaseData[client][0], "SetAnimation", -1, -1, -1);
	DispatchKeyValueFloat(iEntCaseData[client][0], "playbackrate", fOpenSpeedAnim); 
	kv.Rewind();
	bool ex = false;
	int R;
	do{
		if(GetRandomInt(0,100 > 50)) R=0;
		else R=2;
